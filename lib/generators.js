...
function any() {
    ...
    var ret = exports[keys[i]]();

    return funkit.isFunction(ret)? ret(): ret;
}

function number(minValue, maxValue) {
    if(!funkit.isDefined(minValue)) {
        // TODO: figure out which values to use here
        maxValue = 10000;
        minValue = -10000;
    }
    else if(!funkit.isDefined(maxValue)) {
